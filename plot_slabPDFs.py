...
for model in models_w_histonly:
    ...
    npts = len(data.slab.values)

    plotOUT = dirOUT+"plots/pdf.historical."+modelname[mi[0]]+".png"
    print("PDF plot: ",plotOUT)
    fig=plt.figure(figsize=(8, 6))
    plt.gca().set_prop_cycle(plt.cycler('color', plt.cm.jet(np.linspace(0, 1, npts))))
    for slab in range(0,npts):
